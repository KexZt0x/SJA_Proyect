...
router.get('/', (req, res) => {
    const query = `
        SELECT Movimiento.IdMovimiento, DATE_FORMAT(Movimiento.FechaMovimiento, '%Y-%m-%d') AS FechaMovimientoFormateada, Movimiento.SolicitanteEquipo, Movimiento.CorreoSolicitante, Movimiento.TelefonoSolicitante, Movimiento.CantidadProducto, Movimiento.TipoMovimiento, Motivo.DescripcionMovimiento, Producto_Materia_Prima.NombreProducto, Usuario.NombreUsuario
        FROM Movimiento
...
router.get('/:id', (req, res) => {
    const { id } = req.params;
    const query = `
        SELECT Movimiento.IdMovimiento, Movimiento.FechaMovimiento, Movimiento.SolicitanteEquipo, Movimiento.CorreoSolicitante, Movimiento.TelefonoSolicitante, Movimiento.CantidadProducto, Movimiento.TipoMovimiento, Motivo.DescripcionMovimiento, Producto_Materia_Prima.NombreProducto, Usuario.NombreUsuario
        FROM Movimiento
        INNER JOIN Motivo ON Movimiento.IdMotivo = Motivo.IdMotivo
        INNER JOIN Producto_Materia_Prima ON Movimiento.IdProductoMateriaPrima = Producto_Materia_Prima.IdProductoMateriaPrima
        INNER JOIN Usuario ON Movimiento.IdUsuario = Usuario.IdUsuario
...
router.post('/agregar', (req, res) => {
    ...
    const movimiento = {
        ...
        SolicitanteEquipo: req.body.SolicitanteEquipo||'',
...
router.put('/actualizar/:id', (req, res) => {
    ...
    const query = `
        ...
        SolicitanteEquipo='${movimiento.SolicitanteEquipo || ''}',
        CorreoSolicitante='${movimiento.CorreoSolicitante || ''}',
        TelefonoSolicitante='${movimiento.TelefonoSolicitante || ''}',
